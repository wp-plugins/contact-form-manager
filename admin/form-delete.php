...
$formCount = $wpdb->query( 'SELECT * FROM '.$wpdb->prefix.'xyz_cfm_form WHERE id="'.$xyz_cfm_formId.'" LIMIT 0,1' ) ;

if($formCount==0){
	header("Location:".admin_url('admin.php?page=contact-form-manager-manage'));
	exit();
}else{
	$wpdb->query( 'DELETE FROM  '.$wpdb->prefix.'xyz_cfm_form_elements  WHERE form_id="'.$xyz_cfm_formId.'" ' ) ;
	$wpdb->query( 'DELETE FROM  '.$wpdb->prefix.'xyz_cfm_form  WHERE id="'.$xyz_cfm_formId.'" ' ) ;
